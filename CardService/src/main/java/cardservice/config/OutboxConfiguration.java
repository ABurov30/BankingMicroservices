...
public class OutboxConfiguration {
  ...
  @Bean
  public OutboxDispatcher<CardOutboxEventEntity> outboxDispatcher(
      EntityManager entityManager,
      PlatformTransactionManager transactionManager,
      OutboxProperties properties,
      MeterRegistry meters,
      CardOutboxEventRepository repository) {
    Gauge.builder(
            "outbox.pending",
            repository,
            r -> r.countByOutboxEventStatus(OutboxEventStatus.PENDING))
        .tag("outbox", "card_outbox_events")
        .register(meters);
    Gauge.builder(
            "outbox.processing",
            repository,
        ...
        .tag("outbox", "card_outbox_events")
        .register(meters);
    meters.counter("outbox.retry", "outbox", "card_outbox_events");
    var store =
        new JpaOutboxAttemptStore<>(
            entityManager,
            transactionManager,
            CardOutboxEventEntity.class,
            "card_outbox_events",
            properties);
    return new OutboxDispatcher<>(
        store, properties, transactionManager, meters, "card_outbox_events");
  }
