...
public class OutboxConfiguration {
  ...
  @Bean
  public OutboxDispatcher<TransactionOutboxEventEntity> outboxDispatcher(
      EntityManager entityManager,
      PlatformTransactionManager transactionManager,
      OutboxProperties properties,
      MeterRegistry meters,
      TransactionOutboxEventRepository repository) {
    Gauge.builder(
            "outbox.pending",
            repository,
            r -> r.countByOutboxEventStatus(OutboxEventStatus.PENDING))
        .tag("outbox", "transaction_outbox_events")
        .register(meters);
    Gauge.builder(
            "outbox.processing",
            repository,
        ...
        .tag("outbox", "transaction_outbox_events")
        .register(meters);
    meters.counter("outbox.retry", "outbox", "transaction_outbox_events");
    var store =
        new JpaOutboxAttemptStore<>(
            entityManager,
            transactionManager,
            TransactionOutboxEventEntity.class,
            "transaction_outbox_events",
            properties);
    return new OutboxDispatcher<>(
        store, properties, transactionManager, meters, "transaction_outbox_events");
  }
