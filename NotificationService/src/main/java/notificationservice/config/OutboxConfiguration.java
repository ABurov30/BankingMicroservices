...
public class OutboxConfiguration {
  ...
  @Bean
  public OutboxDispatcher<PushNotificationOutboxEventEntity> outboxDispatcher(
      EntityManager entityManager,
      PlatformTransactionManager transactionManager,
      OutboxProperties properties,
      MeterRegistry meters,
      PushNotificationOutboxEventRepository repository) {
    Gauge.builder(
            "outbox.pending",
            repository,
            r -> r.countByOutboxEventStatus(OutboxEventStatus.PENDING))
        .tag("outbox", "push_notification_outbox_events")
        .register(meters);
    Gauge.builder(
            "outbox.processing",
            repository,
        ...
        .tag("outbox", "push_notification_outbox_events")
        .register(meters);
    meters.counter("outbox.retry", "outbox", "push_notification_outbox_events");
    var store =
        new JpaOutboxAttemptStore<>(
            entityManager,
            transactionManager,
            PushNotificationOutboxEventEntity.class,
            "push_notification_outbox_events",
            properties);
    return new OutboxDispatcher<>(
        store, properties, transactionManager, meters, "push_notification_outbox_events");
  }
