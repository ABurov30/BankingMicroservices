...
public class GrpcClientConfig {
    ...
    ManagedChannel notificationChannel(
            @Value("${NOTIFICATION_GRPC_HOST}") String host, @Value("${NOTIFICATION_GRPC_PORT}") int port) {

        return ManagedChannelBuilder.forAddress(host, port).usePlaintext().build();
    }

    @Bean
    NotificationRpcServiceGrpc.NotificationRpcServiceBlockingStub notificationStub(
            ManagedChannel notificationChannel) {
