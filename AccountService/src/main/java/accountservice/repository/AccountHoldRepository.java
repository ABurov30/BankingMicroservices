...
public interface AccountHoldRepository extends JpaRepository<AccountHoldEntity, UUID> {
    ...
    @Query("SELECT ah from AccountHoldEntity ah WHERE ah.status = :reservationStatus AND ah.expiresAt <= :expiresAt ORDER BY ah.createdAt")
    List<AccountHoldEntity> findForUpdateTop50ByReservationStatusAndExpiresAtLessThanEqualOrderByCreatedAtAsc(
