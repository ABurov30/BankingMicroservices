...
class AccountOutboxEventFactoryTest {
  @Test
  void allEventTypesUseOnlyAggregateIdAsKey() {
    UUID aggregateId = UUID.randomUUID();
    for (var type : AccountEventType.values()) {
      var event = AccountOutboxEventFactory.create(aggregateId, type);
      ...
      assertThat(AccountOutboxEventFactory.create(UUID.randomUUID(), type).getEventKey())
