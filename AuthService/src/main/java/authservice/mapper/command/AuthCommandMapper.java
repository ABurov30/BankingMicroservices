...
public interface AuthCommandMapper {
  ...
  default VerifyAuthUserByCodeCommand toVerifyAuthUserByCodeCommand(
      VerifyAuthUserByCodeGrpcRequest request) {
    return new VerifyAuthUserByCodeCommand(
        UUID.fromString(request.getAuthUserId()), request.getVerificationCode());
  }

  default VerifyAuthUserByPrivilegeRoleCommand toVerifyAuthUserByPrivilegeRoleCommand(
      VerifyAuthUserByPrivilegeRoleGrpcRequest request) {
    return new VerifyAuthUserByPrivilegeRoleCommand(
        UUID.fromString(request.getAuthUserId()), Roles.valueOf(request.getRole()));
  }

  default ChangeAuthUserRoleCommand toChangeAuthUserRoleCommand(
      ChangeAuthUserRoleGrpcRequest request) {
